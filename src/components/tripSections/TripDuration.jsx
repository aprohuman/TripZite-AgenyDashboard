...
export default function TripDuration({setStepsCompleted}) {
  ...
  const validateTripDuration = {
      ...
      bookingDeadline: (value) => {
        ...
        if (parseInt(value) < 1 ) return 'Booking deadline can not be 0 or negative.'
        return '';
      },
  };
  ...
  const handleChange = (e) => {
    ...
    setDurationData((prev) => {
      ...
      }else if (name === 'endDate' && prev.startDate && value < prev.startDate) {
    ...
    if (name === 'startDate' && durationData.endDate && value > durationData.endDate) {
    ...
    }else if (name === 'endDate' && durationData.startDate && value < durationData.startDate) {
              ...
              >
                ...
                {field.type === 'date' ? (
                  ...
                  <div className="flex flex-col sm:flex-row w-full justify-start mx-0 sm:mx-10 space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    />
                    <div className="flex ">
                      <button onClick={(e)=>{handleButtons(e,`${field.id}-increment`)}} className="w-full box-border border-1 p-3 border-[#0000004D] rounded-[6px]">
                        +
                      </button>
                      <button onClick={(e)=>{handleButtons(e,`${field.id}-decrement`)}} className="w-full box-border border-1 p-3 border-[#0000004D] rounded-[6px]">
                        -
                      </button>
                    </div>
                  </div>
